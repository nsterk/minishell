...
static void	st_check_filename(t_lexer *lexer, t_token *last);
...
static void	st_check_filename(t_lexer *lexer, t_token *last)
{
	if (!last)
		return ;
	if ((last->type == TOK_REDIR_IN && last->flags ^ F_APPEND) || \
		last->type == TOK_REDIR_OUT)
		lexer->flags |= F_FILENAME;
	else if (last->flags & F_SPACE && last->prev)
	{
		if ((last->prev->type == TOK_REDIR_IN && \
		last->prev->flags ^ F_APPEND) || last->prev->type == TOK_REDIR_OUT)
			lexer->flags |= F_FILENAME;
	}
}
